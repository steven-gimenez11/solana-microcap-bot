...
<title>Solana Microcap Scanner V3</title><style>
...
<h1>Solana Microcap Scanner V3</h1><div class="sub"><span class="ok">● ACTIVE</span> · DRY RUN · búsqueda asimétrica, no garantía de 100x</div>
...
<h2 style="margin-top:20px">Live Executor</h2><div class="metrics"><div class="card">Estado: {{ "ACTIVO" if live.enabled else "DESACTIVADO" }}</div><div class="card">Live abiertas: {{live.open_positions}}</div><div class="card">Máx/posición: ${{live.max_position_usd}}</div></div>
<div class="tokens"><h2>Live Trades</h2><table><thead><tr><th>Symbol</th><th>Status</th><th>Entry</th><th>Multiple</th><th>Remaining raw</th><th>Entry tx</th></tr></thead><tbody>{% for t in live_trades %}<tr><td>{{t.symbol}}</td><td>{{t.status}}</td><td>{{t.entry_price}}</td><td>{{t.multiple}}x</td><td>{{t.remaining_raw_amount}}</td><td>{{(t.entry_signature or '-')[:10]}}</td></tr>{% endfor %}</tbody></table></div>
...
def index():
    ...
    live_stats = scanner.store.live_stats()
    return render_template_string(PAGE, tokens=tokens[:100], m=metrics, p=portfolio, trades=scanner.store.paper_trades()[:50], maxpos=settings.max_open_positions, live=type("L", (), live_stats)(), live_trades=scanner.store.live_trades()[:50])


@app.get("/health")
def health():
    return jsonify({"status": "ok", "mode": "LIVE" if settings.live_mode else "DRY_RUN", "trading_enabled": settings.trading_enabled, "live_mode": settings.live_mode})
...
def live_trades():
    return jsonify(scanner.store.live_trades())


@app.get("/api/live-status")
def live_status():
    status = scanner.store.live_stats()
    status["wallet"] = scanner.live.jupiter.wallet_address if scanner.live.jupiter else None
    status["events"] = scanner.store.live_events(25)
